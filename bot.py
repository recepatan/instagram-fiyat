...
def instagram_metni_olustur(fiyatlar, isimlendirme, baslik, kur_alis, kur_satis):
    """Fiyatları Instagram açıklaması (caption) formatına getirir."""
    metin = f"📢 {baslik}\n"
    ...
    metin += "#altın #altınfiyatları #bingöl #ekonomi #döviz #yatırım"
    return metin

def instagram_paylas(mesaj):
    """Instagram Graph API üzerinden paylaşım yapar."""
    TOKEN = os.getenv("INSTAGRAM_TOKEN")
    USER_ID = os.getenv("INSTAGRAM_USER_ID")
    # Sabit görsel linkiniz (Test için Wikipedia, sonra değiştirin)
    RESIM_URL = "https://upload.wikimedia.org/wikipedia/commons/4/47/PNG_transparency_demonstration_1.png"
    
    try:
        # 1. Adım: Medya Konteyneri Oluştur
...
def run_bot():
    try:
        ...
        # 2. Zaman ve Başlık Ayarları
        istanbul_tz = pytz.timezone('Europe/Istanbul')
        simdi = datetime.now(istanbul_tz)
        
        url_dosya_adi = simdi.strftime("%d-%m-%Y-altin-fiyatlari")
        aylar = {1: "Ocak", 2: "Şubat", 3: "Mart", 4: "Nisan", 5: "Mayıs", 6: "Haziran",
                 7: "Temmuz", 8: "Ağustos", 9: "Eylül", 10: "Ekim", 11: "Kasım", 12: "Aralık"}
        
        gun_str, ay_str, yil_str = simdi.strftime("%d"), aylar[simdi.month], simdi.strftime("%Y")
        saat_str, saat_id = simdi.strftime("%H:%M"), simdi.strftime("%H%M")
        gercek_baslik = f"{gun_str} {ay_str} {yil_str} Altın Fiyatları"
        ...
        for row in rows[1:]:
            if len(row) >= 3:
                ...
                try:
                    fiyatlar[key] = {'alis': float(str(row[1]).replace(',', '.')), 'satis': float(str(row[2]).replace(',', '.'))}
                except: continue
        ...
        # 6. Blogger Yayınlama/Güncelleme
        ...
        if existing_post:
            ...
            print(f"✅ BLOGGER GÜNCELLENDİ: {saat_str}")
        else:
            ...
            gecici = service.posts().insert(blogId=blog_id, body={'title': url_dosya_adi, 'content': ana_icerik, 'labels': ['Altın Fiyatları']}).execute()
            ...
            print(f"🌟 YENİ BLOGGER YAZISI OLUŞTURULDU: {gercek_baslik}")

        # 7. Instagram Paylaşımı (Metin Olarak)
